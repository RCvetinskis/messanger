...
export const ConversationList = ({
  ...
  useEffect(() => {
    ...
    const deleteHandler = (
      deletedConversation: any & { currentUser: string }
    ) => {
      const { conversation, currentUser } = deletedConversation;
      setItems((current) => {
        return [
          ...current.filter(
            (currentCovnersation) => currentCovnersation.id !== conversation.id
          ),
        ];
      });
      if (conversationId === conversation.id) {
        toast.error(`${currentUser} deleted this conversation`);
